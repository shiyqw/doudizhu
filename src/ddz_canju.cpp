...
/*****************************************************************************/
// DFS Process (One Game)
/*****************************************************************************/
bool mc_dfs(int pos, Hand prev_hand, Hand prev_prev_hand) {
  Hand prev;
  ...
  if (local_debug) {
    cout << "dfs for " << pos << endl;
    cout << prev_prev_hand.point << ":" << prev_prev_hand.width
         << "x" << prev_prev_hand.length;
    for (auto card : prev_prev_hand.carry) {
        cout << " " << card;
    }
    cout << endl;
    cout << prev_hand.point << ":" << prev_hand.width
         << "x" << prev_hand.length;
    for (auto card : prev_hand.carry) {
        cout << " " << card;
    }
    cout << endl;
    for (int i = 0; i < 3; ++i) {
              ...
              for (auto combination : combinations) {
                ...
                if (check_valid(string_hand, prev)) {
                  ...
                  if (mc_rem[pos] == 0) {
                    ...
                    auto win_flag = mc_dfs(next_pos, string_hand, prev_hand);
                    if (pos != 1 && !win_flag) {
                      must_win = true;
                    } else if (pos == 1 && win_flag) {
                      must_win = true;
                    }
                  }
                  mc_undo(pos, string_hand);
      ...
      for (int width = 1; width <= mc_shape[pos][i] && width <= 2; ++width) {
        ...
        if (check_valid(pure_hand, prev)) {
          ...
          if (mc_rem[pos] == 0) {
            ...
            auto win_flag = mc_dfs(next_pos, pure_hand, prev_hand);
            if (pos != 1 && !win_flag) {
              must_win = true;
            } else if (pos == 1 && win_flag) {
              must_win = true;
      ...
      if (mc_shape[pos][i] == 4) {
        ...
        if (check_valid(bomb_hand, prev)) {
          ...
          if (mc_rem[pos] == 0) {
            ...
            auto win_flag = mc_dfs(next_pos, bomb_hand, prev_hand);
            if (pos != 1 && !win_flag) {
              must_win = true;
            } else if (pos == 1 && win_flag) {
              must_win = true;
  ...
  if (check_valid(pass_hand, prev)) {
    ...
    if (mc_rem[pos] == 0) {
      ...
      auto win_flag = mc_dfs(next_pos, pass_hand, prev_hand);
      if (pos != 1 && !win_flag) {
        must_win = true;
      } else if (pos == 1 && win_flag) {
        must_win = true;
...
int mc_run(Hand prev_hand, Hand prev_prev_hand, bool prev_pass) {
  ...
  if (rem[0] + rem[1] + rem[2] <= 16) {
    if (prev_pass) {
      prev_prev_hand = Hand();
    }
    auto win_flag = mc_dfs(position, prev_hand, prev_prev_hand);
    if (win_flag) {
      return my_pos;
    } else {
      if (my_pos == 0) {
